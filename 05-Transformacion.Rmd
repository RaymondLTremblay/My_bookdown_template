...
library(data.table)
birth=fread("https://datos.estadisticas.pr/dataset/87620d17-bf63-445c-bb71-aca1a41c9077/resource/ea014209-8716-48a1-ace6-07a22a3c1f9d/download/nchs_births_2001.csv")
...
```{r}
birth |> dplyr::select(BIRMON, CSEX) |> 
  group_by()
unique(birth$STATENAT)
unique(birth$NOCCUR)
unique(birth$RB)
...
```{r,DatosMortandad, echo=FALSE, eval=FALSE}
library(data.table)
...
df%>% 
  dplyr::select(value) %>% 
  mutate(Change_df=value-lag(value, 1)) # El cambio en los valores entre celdas
